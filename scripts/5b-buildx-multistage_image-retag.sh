...
# build temp image
echo "== build temp"
docker buildx build --cache-to=type=inline,mode=max --output=type=docker -t $TEMP_IMAGE_NAME -f "$script_dir/../test-image/Dockerfile-multistage" "$script_dir/../test-image"
# push
echo "== tag"
