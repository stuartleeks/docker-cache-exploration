...
echo "== build temp"
docker build --cache-from $IMAGE_NAME -t $TEMP_IMAGE_NAME -f "$script_dir/../test-image/Dockerfile" "$script_dir/../test-image"
echo "== build features"
docker build --cache-from $FEATURE_IMAGE_NAME  -t $FEATURE_IMAGE_NAME -f "$script_dir/../test-image/Dockerfile-features" "$script_dir/../test-image"
