...
# build temp image
echo "== build temp"
docker build --build-arg BUILDKIT_INLINE_CACHE=1 -t $TEMP_IMAGE_NAME -f "$script_dir/../test-image/Dockerfile-multistage" "$script_dir/../test-image"
# push
echo "== tag"
...
docker build --cache-from $IMAGE_NAME -t $IMAGE_NAME -f "$script_dir/../test-image/Dockerfile-multistage" "$script_dir/../test-image"
